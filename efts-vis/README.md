...
```bash
wget https://raw.githubusercontent.com/csiro-hydrogeology/pyefts_vis/testing/configs/efts_vis_environment.yml
my_env_name=ELA
# my_env_name=etuuser
conda env create -n $my_env_name -f ./efts_vis_environment.yml python=3.7
conda activate $my_env_name 
```

This should be all to get a working environment. If you want to use 'efts_vis' from jupyter lab:
...
where curl
cd c:\tmp
curl -o efts_vis_environment.yml https://raw.githubusercontent.com/csiro-hydrogeology/pyefts_vis/testing/configs/efts_vis_environment.yml
set my_env_name=ELA
conda env create -n %my_env_name% -f efts_vis_environment.yml python=3.7
conda activate %my_env_name% 
```
...
As of January 2019 [etu is on pypi](https://pypi.org/project/efts_vis/). While `pip install efts_vis` might work on some computers, it is _unlikely that all python geospatial dependencies will install_. *We highly recommend you set up a conda environment with all dependent packages* prior to installing efts_vis with pip or from source.

'efts_vis' relies on several external packages, and some can be fiddly to install depending on the version of Python and these external packages. This section thus has fairly prescriptive instructions, given in the hope of limiting the risk of issues.
...
To create the conda environment for efts_vis on Linux:
...
At this point we have installed all the python dependencies efts_vis needs that are available via `conda`.
...
For Python 3.x one needs to install pyqt5 for mayavi, as per [these instructions](https://docs.enthought.com/mayavi/mayavi/installation.html). As of Jan 2019 be aware that there is a [known issue in mayavi visual rendering with pyqt5 as a backend on Linux](https://github.com/enthought/mayavi/issues/656) and 'efts_vis' is affected by this. Nevertheless this is not a commplete blocker for most 'efts_vis' features so installation instructions are kept here.
...
At this point all mandatory dependencies for 'efts_vis' are installed.

### Installing ELA

There are three options to access efts_vis:
...
You may use efts_vis as you prefer; we recomment using "Jupyter Lab" to write notebooks. See the [Jupyter lab doc](https://jupyterlab.readthedocs.io/en/stable/) for official information. 

The following should be enough otherwise to use 'efts_vis'
...
'efts_vis' aims to complement other Python packages for geoscience, in particular for handling bore data . It already depends on the package ['striplog'](https://github.com/agile-geoscience/striplog) and is likely to depend on ['lasio'](https://github.com/kinverarity1/lasio) in the future.
